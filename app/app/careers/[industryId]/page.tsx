...
import { ProfessionCard } from "@/app/components/appLayout";
import { INDUSTRIES, SAMPLE_RECOMMENDED_PROFESSIONS } from "@/lib/careers/constants";
...
function useIndustryData(industryId: string) {
  ...
  const professions = useMemo(
    () => SAMPLE_RECOMMENDED_PROFESSIONS.filter((p) => p.industryId === industryId),
    [industryId]
  );
  ...
  return (
    <AppLayout title={t("header_professions")}>
      <Box sx={{ py: 4, textAlign: "center" }}>
        ...
        <Button component={Link} href="/app/careers" startIcon={<ArrowBackRoundedIcon />}>
          К направлениям
        </Button>
      </Box>
    </AppLayout>
      ...
      <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
        ...
        <Box>
          ...
          <Typography variant="body1" color="text.secondary">
            {t(industry.descKey)} · {t(industry.tagsKey)}
          </Typography>
        </Box>
